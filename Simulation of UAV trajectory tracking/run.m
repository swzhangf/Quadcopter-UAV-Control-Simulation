%Simulation trajectory and drawing
clf
len = length(tout);
...
for i = 1:len
    ...
    [point1_trans,point2_trans,point3_trans,point4_trans]=drone(phi(i),theta(i),psai(i));%»æÖÆËÄÐýÒí
    try
...
legend([h1,h2],["Reference position xd","Actual position x"])
grid on;
subplot(3,1,2)
h3 = plot(tout,yd,"r--","LineWidth",2);hold on;
h4 = plot(tout,y,"b-","LineWidth",2);
legend([h3,h4],["Reference position yd","Actual position y"])
grid on;
subplot(3,1,3)
h5 = plot(tout,zd,"r--","LineWidth",2);hold on;
h6 = plot(tout,z,"b-","LineWidth",2);
legend([h5,h6],["Reference position zd","Actual position z"])
grid on;
%Draw angle image
figure(3);clf;
subplot(3,1,1)
